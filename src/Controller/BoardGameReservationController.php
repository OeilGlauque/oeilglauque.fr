        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if ($reservation->getDateBeg() > $reservation->getDateEnd()) {     
                ...
                if ($reservation->getDateBeg()->diff($reservation->getDateEnd())->days > 30) {
                    ...
                    if ($reservation->getDateBeg() < $edition->getEnd() && $reservation->getDateEnd() > $edition->getStart()) {
                        $this->addFlash('warning', "Pas de réservations possibles pendant la durée du festival !");
                    }else{
                        $overlap = $manager
